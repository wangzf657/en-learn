...
- **语义层**(翻译/讲解/选词)= 执行本 skill 的 agent(即 LLM)逐句完成:LLM 强于规则的地方;失败可整体重做,不污染结构
...
3. `phonetic` 是裸 IPA,**不带首尾斜杠**(前端渲染时自加 `/…/`)
4. `words.w` 必须是 `en` 中出现的**表层拼写**(含缩写/变形,不还原词干)——厂商按 `en` 空格分词打词标签,写原形 `do` 就对不上 `word_scores` 里的 `doing`
...
| `words` | 每句 0–3 个;选本句关键 / 易错发音 / 启蒙高频词,不选 the/a;**同一视频内一个词最多讲一次,名额让给新词** |
| `words.note` | ≤20 字中文,讲"怎么说、什么时候说",不写词典腔 |
| `phonetic` | 美式 IPA(CMUdict 风格),准确;拿不准的词先确认再写,不凭印象造 |
...
脚本逐句比对 cue 数量/时间戳/文本(首要规则)+ 全文守恒 + words 契约;WARN(仅"words > 3 个")不拦;ERROR 必须修完重跑直到全绿——"不对齐/不一致"类 ERROR 说明结构或文本被改过,按 draft 原样恢复。
